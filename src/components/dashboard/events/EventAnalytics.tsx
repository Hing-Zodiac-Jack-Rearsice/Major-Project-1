...
export function EventAnalytics({ event, ticketsLeft }: EventAnalyticsProps) {
  ...
  const revenue = sales.reduce(
    (sum: number, sale: Sale) => sum + (sale.price || 0),
    0
  );
  const attendanceRate =
    tickets.length > 0 ? (attendances.length / tickets.length) * 100 : 0;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {salesVelocity > 0 ? "Tickets sold per day" : "No sales yet"}
              </p>
            </CardContent>
          </Card>
