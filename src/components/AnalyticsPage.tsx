...
const AnalyticsPage: React.FC = () => {
  ...
  const ticketsSoldData = {
    labels:
      analyticsData.ticketsSoldPerEvent?.map((item: any) => item.eventName) ||
      [],
    datasets: [
      {
        label: "Tickets Sold",
  ...
  const salesByEventStartTimeData = {
    labels:
      analyticsData.salesByEventStartTime?.map((item: any) => item.hour) || [],
    datasets: [
      {
        label: "Sales",
        data:
          analyticsData.salesByEventStartTime?.map((item: any) => item.count) ||
          [],
    ...
    datasets: [
      {
        label: "Sales",
        data:
          analyticsData.salesByEventDate?.map((item: any) => item.count) || [],
  ...
  const barData = {
    ...
    datasets: [
      ...
      },
      {
        label: "Revenue",
        data:
          analyticsData.salesOverTime?.map((item: any) => item.revenue) || [],
        backgroundColor: "rgba(54, 162, 235, 0.8)",
        borderColor: "rgba(54, 162, 235, 1)",
        borderWidth: 2,
      },
